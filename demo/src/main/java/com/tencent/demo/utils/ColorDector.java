...
public class ColorDector {
    ...
    /**
     * @param image      e表示目标图像
     * @param contours   表示输入的轮廓组，每一组轮廓由点vector构成，
     * @param contourIdx 指明画第几个轮廓，如果该参数为负值，则画全部轮廓
     * @Description TODO 顏色
    ...
    public String detect(Mat image, List<MatOfPoint> contours, int contourIdx) {
        ...
        //申明计算标准差后的结果数组
        double[] meanScalar = new double[]{scalar.val[0], scalar.val[1], scalar.val[2]};
        ...
        if (redDistance < greenDistance && redDistance < blueDistance) {
            color = "red";
            //绿色距离最小
        } else if (greenDistance < blueDistance) {
            color = "green";
            //蓝色最小
        } else {
            color = "blue";
